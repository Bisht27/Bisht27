<<<<<<< HEAD
<!-- ========================================================= -->
<!--                       HERO SECTION                        -->
<!-- ========================================================= -->

<div align="center">

# 👋 Hi, I'm Deepanshu Bisht

### AI/ML Engineer • Python Backend Developer • DevOps Engineer

**Building AI-powered applications, scalable backend systems & cloud-native infrastructure.**

<br/>

<a href="https://github.com/Bisht27">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>
<a href="https://www.linkedin.com/in/deepanshu-bisht-8a73972a6/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>
<a href="mailto:deepanshubisht738@gmail.com">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>
<a href="YOUR_INSTAGRAM">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white" />
</a>
<a href="YOUR_WEBSITE">
  <img src="https://img.shields.io/badge/Portfolio-000000?style=for-the-badge&logo=vercel&logoColor=white" />
</a>

<br/><br/>

<img src="./Assets/banner.png" width="900" alt="Coding Setup"/>

</div>

---

<!-- ========================================================= -->
<!--                         ABOUT                             -->
<!-- ========================================================= -->

## 🚀 About Me

I'm a **B.Tech Artificial Intelligence & Machine Learning student** focused on building practical software systems across **AI/ML, backend engineering, DevOps and cloud infrastructure**.

- 🔭 Currently building **AI-powered and backend systems**
- 💻 Strong focus on **Python, FastAPI, Machine Learning & REST APIs**
- ☁️ Working with **AWS, Docker, Kubernetes & CI/CD**
- ⚙️ Interested in **cloud infrastructure, automation & system reliability**
- 🤖 Exploring **LLMs, RAG, Prompt Engineering & Vector Databases**
- 🧠 Currently improving **Data Structures & Algorithms + System Design**
- 🌱 Learning **Django, React, PostgreSQL, MongoDB & Redis**
- 🛠️ Enjoy taking projects from **development → containerization → deployment**
- 💬 Ask me about **Python, FastAPI, Docker, AWS, Linux, CI/CD & Machine Learning**
- 🎯 Goal: Become a strong **AI/ML + Backend + Cloud Engineer**

---

<!-- ========================================================= -->
<!--                     TECH STACK                            -->
<!-- ========================================================= -->

## 🧰 Tech Stack

### 🐍 Backend & Programming

<p align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=python,java,javascript,fastapi,django,nodejs,express" />
</a>
</p>

### 🎨 Frontend

<p align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=html,css,js,react" />
</a>
</p>

### ☁️ Cloud & DevOps

<p align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=aws,azure,docker,kubernetes,jenkins,githubactions,linux,bash" />
</a>
</p>

### 🗄️ Databases

<p align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=mysql,postgresql,mongodb,redis" />
</a>
</p>

### 🤖 AI / Machine Learning

<p align="center">
<img src="https://img.shields.io/badge/Machine%20Learning-102230?style=for-the-badge&logo=scikit-learn&logoColor=F7931E" />
<img src="https://img.shields.io/badge/Scikit--Learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white" />
<img src="https://img.shields.io/badge/NLP-412991?style=for-the-badge" />
<img src="https://img.shields.io/badge/Generative%20AI-000000?style=for-the-badge" />
<img src="https://img.shields.io/badge/LLMs-6E56CF?style=for-the-badge" />
<img src="https://img.shields.io/badge/RAG-4B5563?style=for-the-badge" />
<img src="https://img.shields.io/badge/Prompt%20Engineering-111827?style=for-the-badge" />
<img src="https://img.shields.io/badge/Vector%20Databases-2563EB?style=for-the-badge" />
</p>

### 🔧 Tools

<p align="center">
<a href="https://skillicons.dev">
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman" />
</a>
</p>

---

<!-- ========================================================= -->
<!--                       FEATURED PROJECTS                    -->
<!-- ========================================================= -->

## ⭐ Featured Projects

### 🖥️ Enterprise AIOps Platform

Enterprise-grade infrastructure monitoring and asset management platform.

**Tech:** Python • FastAPI • MySQL • Docker • REST APIs • JWT • RBAC • GitHub Actions • Linux

- 📊 System & hardware telemetry monitoring
- 🔐 JWT authentication and role-based access control
- 💓 Heartbeat-based monitoring
- 🖥️ CPU, memory, disk, OS, network & hardware monitoring
- 🔔 Notification management
- 🚀 Automated deployment with Docker & GitHub Actions

<a href="https://github.com/Bisht27/Enterprise-AIOps-Platform">
  <img src="https://img.shields.io/badge/View%20Project-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

---

### 🏥 Medicare — Doctor Appointment Booking System

Full-stack healthcare appointment booking application.

**Tech:** Node.js • Express.js • MySQL • Docker • Jenkins • Kubernetes • AWS EC2

- 🏗️ Full-stack healthcare application
- 🗄️ MySQL database integration
- 🐳 Docker containerization
- 🔄 Jenkins CI/CD automation
- ☸️ Kubernetes deployment
- ☁️ AWS EC2 deployment

<a href="https://github.com/Bisht27/medicare">
  <img src="https://img.shields.io/badge/View%20Project-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

---

### ☀️ Solar Energy Prediction System

Machine learning system for predicting solar energy output.

**Tech:** Python • Scikit-learn • Pandas • NumPy • Matplotlib

- 📊 Data preprocessing
- ⚙️ Feature engineering
- 🤖 Machine learning model development
- 📈 Model evaluation
- 🔬 Iterative model tuning

<a href="https://github.com/Bisht27">
  <img src="https://img.shields.io/badge/View%20Projects-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

---

<!-- ========================================================= -->
<!--                       EXPERIENCE                          -->
<!-- ========================================================= -->

## 💼 Experience

### DevOps Intern — 50Hertz Ltd

**Jun 2026 – Aug 2026**

Worked on cloud-native deployment and DevOps automation.

- 🐳 Containerized applications using Docker
- ☸️ Worked with Kubernetes
- 🔄 Automated CI/CD using Jenkins & GitHub Actions
- ☁️ Deployed workloads on AWS EC2 & Microsoft Azure
- 🐧 Configured Linux servers
- ⚙️ Worked with Docker Compose and Git workflows

---

<!-- ========================================================= -->
<!--                    GITHUB ANALYTICS                        -->
<!-- ========================================================= -->

## 📊 GitHub Analytics

<div align="center">

<img height="180" src="https://github-readme-stats.vercel.app/api?username=Bisht27&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" />
<img height="180" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Bisht27&layout=compact&theme=tokyonight&hide_border=true" />

<br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Bisht27&theme=tokyonight&hide_border=true" />

<br/>

<img src="https://github-profile-trophy.vercel.app/?username=Bisht27&theme=tokyonight&no-frame=true&row=1&column=7" />

</div>

---

<!-- ========================================================= -->
<!--                    CURRENTLY LEARNING                     -->
<!-- ========================================================= -->

## 📚 Currently Learning

```text
DSA & Problem Solving
        ↓
System Design
        ↓
Advanced Backend Development
        ↓
LLMs & RAG
        ↓
Vector Databases
        ↓
Cloud Architecture
        ↓
Production DevOps & SRE
```

---

<div align="center">

⭐️ **Thanks for visiting my profile — let's build something great together!**

</div>
=======
## Hi there 👋

<!--
**Bisht27/Bisht27** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
>>>>>>> 281533a859fb4e3571968505dd26faf872d0d409
